...
function BentoCard({ icon, title, subtitle, href = '#', span }: BentoCardProps) {
  const content = (
    <>
      {!span && (
...
export function BentoGrid() {
  return (
    <section className="grid grid-cols-2 gap-3 px-6">
      <BentoCard icon="💰" title="환율 계산" subtitle="1 AUD = 880 KRW" href="/tools/exchange" />
      <BentoCard icon="🎒" title="짐싸기 체크" subtitle="필수품 리스트" href="/tools/packing" />
      <BentoCard
        icon="🛡️"
        title="워홀 보험 특가 📢"
